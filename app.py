...
async def generate_embeddings(text_chunks, max_retries=3, delay=5):
    client = initialize_mistral()
    for attempt in range(max_retries):
        try:
            response = await client.embeddings(model="mistral-embed", input=text_chunks)
...
async def process_query(user_query, text_chunks, faiss_index, max_retries=3, delay=5):
    query_embedding = np.array([await generate_embeddings([user_query])[0].embedding])
    ...
    {user_query}
    """
    return await generate_response(query_prompt, max_retries, delay)

# Function to communicate with Mistral with retry logic
async def generate_response(input_text, max_retries=3, delay=5, model="mistral-large-latest"):
    client = initialize_mistral()
    for attempt in range(max_retries):
        try:
...
def main():
    ...
    embeddings = asyncio.run(generate_embeddings(text_chunks))
    faiss_index = setup_faiss(embeddings)

    user_input = st.text_input("Ask a question about the policy:")
    if user_input:
        response = asyncio.run(process_query(user_input, text_chunks, faiss_index))
